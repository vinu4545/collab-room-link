...
async function getSupabaseAdminOrNull() {
	try {
		...
		return (await import("./client.server-KzwUIAkW.mjs")).supabaseAdmin ?? null;
	} catch {
		return null;
	}
}
...
var checkUsername = createServerFn({ method: "POST" }).validator((data) => usernameSchema.parse(data)).handler(checkUsername_createServerFn_handler, async ({ data }) => {
	...
	if (supabaseAdmin) {
		const { data: row } = await supabaseAdmin.from("terminals").select("id").ilike("owner_username", data.username).maybeSingle();
...
var createTerminal = createServerFn({ method: "POST" }).validator((data) => createSchema.parse(data)).handler(createTerminal_createServerFn_handler, async ({ data }) => {
	const { hashPassword, validatePassword } = await import("./terminal.server-DM_Vs-44.mjs");
	const supabaseAdmin = await getSupabaseAdminOrNull();
	...
	if (supabaseAdmin) {
		const { data: existing } = await supabaseAdmin.from("terminals").select("id").ilike("owner_username", data.username).maybeSingle();
		if (existing) return {
			ok: false,
			code: "username_taken",
		...
		const { data: terminal, error } = await supabaseAdmin.from("terminals").insert({
			name: data.terminalName,
			owner_username: data.username,
			password_hash
		}).select("id, name, owner_username").single();
		if (error || !terminal) {
		...
		const { data: member } = await supabaseAdmin.from("terminal_members").insert({
...
var joinTerminal = createServerFn({ method: "POST" }).validator((data) => joinSchema.parse(data)).handler(joinTerminal_createServerFn_handler, async ({ data }) => {
	const { verifyPassword } = await import("./terminal.server-DM_Vs-44.mjs");
	const supabaseAdmin = await getSupabaseAdminOrNull();
	if (supabaseAdmin) {
		const { data: terminal } = await supabaseAdmin.from("terminals").select("id, name, owner_username, password_hash").ilike("owner_username", data.username).maybeSingle();
...
var getWorkspace = createServerFn({ method: "POST" }).validator((data) => sessionSchema.parse(data)).handler(getWorkspace_createServerFn_handler, async ({ data }) => {
	const { requireMember } = await import("./terminal.server-DM_Vs-44.mjs");
	const supabaseAdmin = (await requireMember(data.terminalId, data.memberId)).supabaseAdmin ?? null;
	if (supabaseAdmin) {
		await supabaseAdmin.from("terminal_members").update({ last_seen: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", data.memberId);
		const [terminal, members, messages, files] = await Promise.all([
			...
			supabaseAdmin.from("terminal_members").select("id, display_name, is_owner, last_seen").eq("terminal_id", data.terminalId).order("created_at"),
			supabaseAdmin.from("terminal_messages").select("id, author, body, created_at").eq("terminal_id", data.terminalId).order("created_at").limit(300),
			supabaseAdmin.from("terminal_files").select("id, uploader, file_name, file_size, mime_type, created_at").eq("terminal_id", data.terminalId).order("created_at", { ascending: false })
		]);
...
var uploadTerminalFile = createServerFn({ method: "POST" }).validator((data) => uploadSchema.parse(data)).handler(uploadTerminalFile_createServerFn_handler, async ({ data }) => {
	...
	if (supabaseAdmin) {
		...
		const { error } = await supabaseAdmin.storage.from("terminal-files").upload(path, binary, {
	...
	await addFileRecord({
		terminal_id: data.terminalId,
		uploader: member?.display_name ?? "unknown",
		file_name: data.fileName,
		file_size: binary.byteLength,
		mime_type: data.mimeType,
	...
	await insertMessage({
		terminal_id: data.terminalId,
		author: "system",
		body: `${member?.display_name ?? "Someone"} uploaded ${data.fileName}.`
	});
	return { ok: true };
});
...
var getFileLink = createServerFn({ method: "POST" }).validator((data) => fileLinkSchema.parse(data)).handler(getFileLink_createServerFn_handler, async ({ data }) => {
	...
	const supabaseAdmin = (await requireMember(data.terminalId, data.memberId)).supabaseAdmin ?? null;
	if (supabaseAdmin) {
		const { data: file } = await supabaseAdmin.from("terminal_files").select("storage_path").eq("id", data.fileId).eq("terminal_id", data.terminalId).maybeSingle();
		if (!file) return {
			ok: false,
			url: null
		};
		const { data: signed } = await supabaseAdmin.storage.from("terminal-files").createSignedUrl(file.storage_path, 600);
		return {
			ok: true,
...
var leaveTerminal = createServerFn({ method: "POST" }).validator((data) => sessionSchema.parse(data)).handler(leaveTerminal_createServerFn_handler, async ({ data }) => {
	...
	const { deleteMember, insertMessage, getMember } = await import("./localStorage.server-DgrQmuv2.mjs");
	const memberRec = await getMember(data.terminalId, data.memberId);
	if (memberRec) {
		await insertMessage({
			terminal_id: data.terminalId,
			author: "system",
			body: `${memberRec.display_name} disconnected.`
		});
