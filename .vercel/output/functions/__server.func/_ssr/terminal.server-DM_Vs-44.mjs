//#region node_modules/.nitro/vite/services/ssr/assets/terminal.server-DM_Vs-44.js
var ITERATIONS = 1e5;
function toHex(buf) {
...
async function requireMember(terminalId, memberId) {
	try {
		...
		const { data: member } = await supabaseAdmin.from("terminal_members").select("id, display_name, terminal_id").eq("id", memberId).eq("terminal_id", terminalId).maybeSingle();
		if (member) return {
			supabaseAdmin,
			member
		};
