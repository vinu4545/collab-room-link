...
function caseInsensitiveEq(a: string, b: string) {
    return a.toLowerCase() === b.toLowerCase();
}

export async function findTerminalByUsername(username: string) {
    const db = await readDB();
    return db.terminals.find((t) => caseInsensitiveEq(t.owner_username, username));
}
...
export async function getWorkspaceData(terminalId: string) {
    ...
    return { terminal, members, messages, files };
}
...
function cryptoRandomUUID() {
    // Use global crypto if available (node 19+), otherwise fallback
    try {
        // @ts-ignore
        return crypto.randomUUID();
    } catch (e) {
        return ([1e7] as any).toString();
    }
}
...
export async function saveUploadFile(terminalId: string, safeName: string, binary: Uint8Array) {
    ...
    const filename = `${cryptoRandomUUID()}-${safeName}`;
